...
import { Plus, Edit, FileText, Sparkles, BookOpen, Eye, EyeOff, Tag, Pin, ArrowUp, ArrowDown, Clock, Trash2 } from "lucide-react";
...
export function AdminPostsTable({ initialPosts, initialGuides = [], activeType, counts }: AdminPostsTableProps) {
  ...
  const [selectedItemForPin, setSelectedItemForPin] = useState<{ id: string; title: string; isPinned: boolean; pinUntil?: any; isGuide?: boolean } | null>(null);
  const [isPinModalOpen, setIsPinModalOpen] = useState(false);

  const isGuideTab = activeType === "GUIDE";

  const handleOpenPinModal = (item: any, isGuide: boolean) => {
  ...
  const handleSavePin = async (isPinned: boolean, pinUntil: string | null) => {
    ...
    const { id, isGuide } = selectedItemForPin;

    try {
      const endpoint = isGuide ? `/api/admin/guides/${id}` : `/api/admin/posts/${id}`;
      ...
      if (isGuide) {
        const updated = guides.map((g) =>
          g.id === id ? { ...g, isPinned, pinUntil } : g
        );
        setGuides(sortPinnableContents(updated));
      } else {
        const updated = posts.map((p) =>
          p.id === id ? { ...p, isPinned, pinUntil } : p
        );
  ...
  return (
    <div className="space-y-6">
      {/* Category Tabs (Strictly Category-only, No 'ALL') */}
      <div className="flex flex-wrap items-center gap-2">
        {tabs.map((tab) => {
          const isActive = activeType === tab.val;
          return (
            <Link
              key={tab.val}
              href={`/admin/posts?type=${tab.val}`}
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 max-w-xs sm:max-w-sm">
                              <div className="flex items-center gap-1.5">
                                ...
                                <h3 className="font-bold text-sm text-zinc-900 line-clamp-1">{guide.title}</h3>
                              </div>
                              <p className="text-[11px] text-zinc-400 line-clamp-1 mt-0.5">{guide.summary || guide.content?.substring(0, 60)}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 text-zinc-700 font-medium">
                          {guide.product?.name || <span className="text-zinc-400 text-[11px]">연결 없음</span>}
                        </td>

                        {/* ViewCount & Date */}
                        <td className="px-4 py-4 text-[11px] text-zinc-500">
                          <div>{guide.viewCount || 0}회</div>
                          <div className="text-zinc-400">{new Date(guide.createdAt).toLocaleDateString()}</div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          ...
                          >
                            {guide.published ? <Eye className="w-3 h-3 text-emerald-600" /> : <EyeOff className="w-3 h-3 text-zinc-400" />}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <Link 
                              href={`/admin/posts/${post.id}/edit?returnType=${activeType}`} 
                              className="px-2.5 py-1.5 bg-zinc-900 text-white hover:bg-zinc-800 rounded-lg text-xs font-bold transition-colors inline-block shadow-2xs"
                            >
                              수정
                            </Link>
      ...
      {selectedItemForPin && (
        <PinModal
          ...
          title={selectedItemForPin.isGuide ? "가이드 1순위 추천 고정 설정" : "콘텐츠 1순위 상단 고정 설정"}
