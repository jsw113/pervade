...
export default function NewProductPage() {
  ...
  const [legalUsageForm, setLegalUsageForm] = useState("예) 일반방향·탈취제품 > 탈취제 > 일반용 (물체용)방향·탈취제품 > 탈취제 > 자동차용(실내용) > 특수목적용·세정제품 > 세정제 > 일반용 (건물 바닥용)세정제품 > 세정제 > 일반용 (렌지후드용)세정제품 > 세정제 > 일반용 (변기용)세정제품 > 세정제 > 일반용 (오븐용)세정제품 > 세정제 > 일반용 (욕실용)용 (실내공간용), 자동차용 (실내용) / 액체형 (라벨 & 상세이미지와 동일하게 기재)");
  const [legalExpiryDate, setLegalExpiryDate] = useState("해당 없음");
  const [legalWeightCapacity, setLegalWeightCapacity] = useState("500ml");
  const [legalEffect, setLegalEffect] = useState("상품 상세페이지 참조");
  const [legalManufacturerOrigin, setLegalManufacturerOrigin] = useState("제조사 : (주)퍼베이드 / 제조국 : 대한민국");
  const [legalChildProtection, setLegalChildProtection] = useState("어린이보호포장 비대상");
  const [legalIngredients, setLegalIngredients] = useState("에탄올, 정제수, 천연향료");
  const [legalCautions, setLegalCautions] = useState("밀폐된 공간에서 사용 시 환기를 충분히 하시오. 내용물을 마시거나, 내용물이 눈 또는 피부에 닿을 경우 인체에 심각한 손상을 입힐 수 있으니 주의하시오. 어린이 손에 닿지 않는 곳에 보관하시오. 사람 또는 동물에 직접 사용(분사)하지 마시오. 표시사항에 기재된 제품의 용도 외에는 사용하지 마시오. 다른 제품과 섞어 사용할 경우 인체에 치명적인 손상을 입힐 수 있으니 섞어 사용하지 마시오. 공기 소독(연무 소독, 고압분사용 소독장비 활용하는 경우 포함)의 용도 사용을 금지하오니, 물체 표면에만 사용하시오. 어린이보호포장이 적용되지 아니한 제품으로 어린이의 손이 닿지 않는 곳에 보관하시오. 화기를 가까이 하지 마시오. 직사광선을 피하여 보관하시오. 광택이 있는 물체 혹은 섬유에 사용 시 변색, 탈색 테스트 후 사용하십시오. 제품을 세워서 보관하십시오.");
  const [legalSafetyCertNo, setLegalSafetyCertNo] = useState("CB24-13-0521");
  const [legalCsPhone, setLegalCsPhone] = useState("070-7756-3668");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const parsedOrigPriceNum = originalPrice && originalPrice.trim() !== ""
